...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(
            self, columns=("name", "tel", "email", "salary"), height=45, show="headings"
        )

        self.tree.column("name", width=30, anchor=tk.CENTER)
        self.tree.column("tel", width=300, anchor=tk.CENTER)
...
class DB:
    ...
    def insert_data(self, name, tel, salary, email):
        self.cursor.execute(
            """INSERT INTO db(name, tel, salary, email) VALUES(?, ?, ?)""", (name, tel, salary, email)
        )
        self.conn.commit()
